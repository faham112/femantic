            ...
            {[7, 14, 30].map((d) => (
              <button key={d} onClick={() => setDays(d)}
                className={`px-3 py-1.5 text-sm rounded-lg font-medium transition ${
                  days === d ? "bg-primary-600 text-white" : "bg-white border border-slate-200 text-slate-600 hover:bg-slate-50"
                }`}>{d}d</button>
            ))}
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
                ...
                {stats.top_pages.length === 0 ? <p className="text-sm text-slate-500">No data yet</p> : (
                  <ul className="space-y-3">{stats.top_pages.map((p, i) => (
              ...
              <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
                ...
                {stats.top_referrers.length === 0 ? <p className="text-sm text-slate-500">No data yet</p> : (
                  <ul className="space-y-3">{stats.top_referrers.map((r, i) => (
                    <li key={i} className="flex justify-between text-sm">
