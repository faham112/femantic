...
def calculate_traffic_score(user_agent: Optional[str], path: str, referrer: Optional[str]) -> tuple:
    ...
    if not user_agent:
        return 0.1, "bot", True
    ua = user_agent.lower()
    bot_patterns = [r"bot", r"crawl", r"spider", r"slurp", r"facebookexternalhit", r"bingpreview", r"googlebot", r"yandex", r"baidu", r"duckduck", r"semrush", r"ahrefs", r"petalbot", r"bytespider"]
    ...
    try:
        ...
        if kind in ("heartbeat", "event"):
            try:
                ...
                db.add(Event(website_id=website.id, session_id=event.session_id, visitor_id=event.visitor_id or event.session_id, event_name=event.event_name or kind, event_data=event.event_data or {"path": event.path}))
                db.commit()
        ...
        sess_pk = None
        try:
            sess = upsert_session(db, website.id, event.visitor_id or event.session_id, event.session_id, country=country, device=device, browser=browser, os=os_name, is_pageview=True)
        ...
        try:
            ...
            db.rollback()
            log.warning("pageview retry without session: %s", e)
...
def _period_stats(db: Session, website_id: int, since: datetime, until: datetime):
    views = db.query(PageView).filter(PageView.website_id == website_id, PageView.created_at >= since, PageView.created_at < until, PageView.traffic_label == "human").count()
    users = db.query(func.count(func.distinct(PageView.visitor_id))).filter(PageView.website_id == website_id, PageView.created_at >= since, PageView.created_at < until, PageView.traffic_label == "human").scalar() or 0
    bounce = 0.0
    sess_q = db.query(func.count(func.distinct(PageView.session_id))).filter(PageView.website_id == website_id, PageView.created_at >= since, PageView.created_at < until, PageView.traffic_label == "human", PageView.session_id.isnot(None)).scalar() or 0
    if sess_q:
        singles = db.query(PageView.session_id).filter(PageView.website_id == website_id, PageView.created_at >= since, PageView.created_at < until, PageView.traffic_label == "human", PageView.session_id.isnot(None)).group_by(PageView.session_id).having(func.count(PageView.id) == 1).count()
        bounce = round((singles / sess_q) * 100, 1)
    elif users:
        singles = db.query(PageView.visitor_id).filter(PageView.website_id == website_id, PageView.created_at >= since, PageView.created_at < until, PageView.traffic_label == "human").group_by(PageView.visitor_id).having(func.count(PageView.id) == 1).count()
...
def get_stats(website_id: int, days: int = 7, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def get_series(website_id: int, days: int = 14, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    if not user_can_access_website(db, current_user, website_id):
        raise HTTPException(status_code=404, detail="Website not found")
    days = max(1, min(days, 90))
    ...
    rows = db.query(cast(PageView.created_at, Date).label("d"), func.count(PageView.id)).filter(PageView.website_id == website_id, PageView.traffic_label == "human", PageView.created_at >= datetime.combine(prev_start, datetime.min.time())).group_by("d").all()
    by_day = {r[0]: r[1] for r in rows}
    def fill(from_day: date, n: int):
        return [int(by_day.get(from_day + timedelta(days=i), 0)) for i in range(n)]
    return {"current": fill(start, days), "previous": fill(prev_start, days), "days": days}


DIMS = {
    "path": PageView.path, "content": PageView.path, "referrer": PageView.referrer, "country": PageView.country,
    "device": PageView.device, "browser": PageView.browser, "os": PageView.os, "utm_source": PageView.utm_source,
    "utm_medium": PageView.utm_medium, "utm_campaign": PageView.utm_campaign, "utm_term": PageView.utm_term,
    ...
    "traffic": PageView.traffic_label, "hostname": PageView.referrer,
}


@router.get("/breakdown/{website_id}")
def breakdown(website_id: int, dim: str = "path", days: int = 14, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    if not user_can_access_website(db, current_user, website_id):
        raise HTTPException(status_code=404, detail="Website not found")
    days = max(1, min(days, 90))
    since = datetime.utcnow() - timedelta(days=days)
    if dim in ("entry", "exit"):
        visitors = db.query(PageView.visitor_id, func.min(PageView.id) if dim == "entry" else func.max(PageView.id)).filter(PageView.website_id == website_id, PageView.created_at >= since, PageView.traffic_label == "human", PageView.visitor_id.isnot(None)).group_by(PageView.visitor_id).all()
        ids = [i for _, i in visitors if i]
        rows = []
        if ids:
            rows = db.query(PageView.path, func.count(PageView.id).label("views")).filter(PageView.id.in_(ids)).group_by(PageView.path).order_by(desc("views")).limit(50).all()
        total = sum(v for _, v in rows) or 1
        return {"dim": dim, "rows": [{"label": p or "/", "views": v, "pct": round(v * 100 / total, 1)} for p, v in rows]}
    col = DIMS.get(dim, PageView.path)
    q = db.query(col, func.count(PageView.id).label("views")).filter(PageView.website_id == website_id, PageView.created_at >= since)
    if dim != "traffic":
...
def list_events(website_id: int, days: int = 14, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    if not user_can_access_website(db, current_user, website_id):
        raise HTTPException(status_code=404, detail="Website not found")
    since = datetime.utcnow() - timedelta(days=max(1, min(days, 90)))
    rows = db.query(Event.event_name, func.count(Event.id).label("count")).filter(Event.website_id == website_id, Event.created_at >= since, Event.event_name != "heartbeat").group_by(Event.event_name).order_by(desc("count")).limit(50).all()
    recent = db.query(Event).filter(Event.website_id == website_id, Event.created_at >= since, Event.event_name != "heartbeat").order_by(Event.created_at.desc()).limit(25).all()
    return {"totals": [{"name": n, "count": c} for n, c in rows], "recent": [{"name": e.event_name, "visitor_id": e.visitor_id, "created_at": e.created_at.isoformat() if e.created_at else None} for e in recent]}
